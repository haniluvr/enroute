...
export const RecommendToUsersModal = ({ isOpen, onClose, jobTitle, employer, jobLink }: Props) => {
  ...
  const handleRecommend = async () => {
    ...
    try {
      ...
      alert(`Successfully sent recommendations to ${selectedUsers.size} users!`);
      setSelectedUsers(new Set());
      onClose();
    } catch (error: any) {
      alert(`Error sending recommendations: ${error.message}`);
    } finally {
      setIsPushing(false);
    }
  };
      ...
      <div className="bg-[#161a29] border border-white/10 rounded-3xl w-full max-w-2xl overflow-hidden shadow-2xl flex flex-col max-h-[90vh]" onClick={e => e.stopPropagation()}>
        <div className="px-6 py-5 border-b border-white/10 flex justify-between items-center bg-[#111524]">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="font-bold text-lg text-white leading-tight">Recommend to Users</h3>
        ...
        <div className="p-6 border-t border-white/10 bg-[#111524] flex items-center justify-between shrink-0">
          ...
          >
            ...
            {isPushing ? 'Sending...' : 'Recommend to Users'}
          </button>
        </div>
      </div>
    </div>
